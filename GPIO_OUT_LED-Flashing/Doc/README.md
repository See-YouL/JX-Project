...
其中1为复位GPIO指定外设, 2为复位AFIO外设, 3为初始化GPIO外设, 4为初始化GPIO外设结构体,
5为读取指定引脚的输入状态, 6为读取整个GPIO的输入状态, 7为读取指定引脚的输出状态,
8为读取整个GPIO的输出状态, 9为设置指定引脚的输出状态为高电平,
10为设置指定引脚的输出状态为低电平, 11为设置指定引脚的输出状态为指定电平,
12为设置整个GPIO的输出状态.
